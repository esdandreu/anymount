...
impl StatusCommand {
    pub fn execute(&self) -> crate::cli::Result<()> {
        ...
        self.execute_with_probe_writer(provider_daemon_ready, &mut out)
    }

    fn execute_with_probe_writer(
        &self,
        ...
        for name in names {
            match cd.read(&name) {
                ...
                Ok(cfg) => {
                    let storage = storage_type_label(&cfg.storage);
                    let path = cfg.path.display().to_string();
                    let running = probe(&name);
...
fn storage_type_label(storage: &StorageConfig) -> &'static str {
    match storage {
...
mod tests {
    ...
    fn empty_config_dir_prints_message() {
        ...
        let mut buf = Vec::new();
        cmd.execute_with_probe_writer(|_| false, &mut buf)
            .expect("status");
    ...
    fn probe_true_yields_running() {
        ...
        let mut buf = Vec::new();
        cmd.execute_with_probe_writer(|_| true, &mut buf)
            .expect("status");
        let s = String::from_utf8(buf).expect("utf8");
